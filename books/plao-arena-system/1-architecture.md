...
このシリーズでは、「アンテナを変えたら良くなった気がする」という主観から脱却し、ノイズ環境下で構成変更の効果を客観的に分離する設計を段階的に解説します。これはADS-Bに限らず、**「ノイズの多い現実環境で改善を証明する」という、IoT・機械学習評価・Web A/Bテスト・SREなど、あらゆる計測系に共通する問題**を扱っています。
...
ここで問題を明確にします。私がやりたいのは「数値が上がったことを確認する」ことではなく、**「構成変更によって数値が上がったことを証明する」**ことです。

観測と因果は違います。**観測はログを見れば誰でもできます。しかし、因果を語るには「設計」が必要になります。** 外的要因の影響を分離する仕組みを、上流の指標設計から作り込む必要があるのです。
...
- **ARENA（Consumer）**: 統計処理に特化した解析エンジン。構成変更の効果と外的要因の影響を分離するため、多角的な統計モデルを用います。

## まとめ

最大距離は目立ちますが、**「再現可能で、比較可能で、持続的改善に敏感な指標」**こそが改善の証明に使える真の指標です。

本章ではその第一歩として、指標の再定義と評価対象を明確にする重要性を整理しました。

**「何を測るか」を誤れば、その後の統計はすべて無意味になります。** 第2章では、この前提を支えるログ基盤の設計に踏み込みます。
